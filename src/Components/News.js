...
export default class News extends Component {
  ...
  constructor(props) {
    ...
    this.string = this.props.category;
    document.title = "News Spot - " + this.string[0].toUpperCase() + this.string.slice(1);
  }

  async updateNews() {
    this.setState({ loading: true });
    await fetch(`https://newsapi.org/v2/top-headlines?country=in&category=${this.props.category}&apiKey=${this.props.apiKey}&page=${this.state.page}&pageSize=${this.props.pageSize}`).then((res) => {
      res.json().then((data) => {
  ...
  async componentDidMount() {
    ...
    await fetch(`https://newsapi.org/v2/top-headlines?country=in&category=${this.props.category}&apiKey=${this.props.apiKey}&page=1&pageSize=${this.props.pageSize}`).then((res) => {
      res.json().then((data) => {
        // console.log(data.articles);
        this.setState({
          articles: data.articles,
  ...
  handleNext = () => {
    ...
    if ((this.state.page) <= Math.ceil(this.state.totalPage / this.props.pageSize)) {
      this.updateNews();
    }
  }
    ...
    await fetch(`https://newsapi.org/v2/top-headlines?country=in&category=${this.props.category}&apiKey=${this.props.apiKey}&page=${this.state.page}&pageSize=${this.props.pageSize}`).then((res) => {
      res.json().then((data) => {
        ...
        this.setState({
          ...
          totalPage: this.state.articles.length,
          loading: false
        })
      })
    });
